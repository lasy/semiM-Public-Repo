...
hsmm$states = data.frame(
  ...
  group_sojourn_time_scale = c("Ovulatory","Ovulatory","Ovulatory","-","-","-","Ovulatory",
                               "-","Pregnancy","Birth or loss",
                               "-","-","Pregnancy","Birth or loss",
                               "Anovulatory","Anovulatory","Anovulatory"),
...
pdf(file = str_c(IO$panels,"model_graph.pdf"), width = viz$full_width/3, height = viz$full_width/3)
par(mar = c(0,0,0,0))
plot(g,# edge.arrow.size=0.75, 
     vertex.size=60, vertex.frame.color="white", 
     vertex.label.color="white", vertex.label.family  = "sans", 
     vertex.label.font = 2, vertex.label.cex = 1.2,
     vertex.color = hsmm$states$colors[match(V(g)$name,  hsmm$states$abbr)],
     layout = hsmm$states$l,
     xlim = range(hsmm$states$l[,1])+c(-1,1)*0.2,
     ylim = range(hsmm$states$l[,2])+c(-1,1)*0.2,
     margin = 1,
     rescale = FALSE
)
dev.off()
...
hsmm$sojourn[hsmm$states$abbr == "lE",30:Nd] = 0
hsmm$sojourn[hsmm$states$abbr == "lE",] = hsmm$sojourn[hsmm$states$abbr == "lE",]/sum(hsmm$sojourn[hsmm$states$abbr == "lE",])
...
hsmm$sojourn[hsmm$states$abbr == "Lut",18:Nd] = 0
hsmm$sojourn[hsmm$states$abbr == "Lut",] = hsmm$sojourn[hsmm$states$abbr == "Lut",]/sum(hsmm$sojourn[hsmm$states$abbr == "Lut",])
...
hsmm$sojourn[hsmm$states$abbr == "PP",1:18] = 0
hsmm$sojourn[hsmm$states$abbr == "PP",] = 
  hsmm$sojourn[hsmm$states$abbr == "PP",]/sum(hsmm$sojourn[hsmm$states$abbr == "PP",])
...
sojourn_long = sojourn_long %>% mutate(
  color = hsmm$states$colors[match(State, hsmm$states$abbr)],
  group = hsmm$states$group_sojourn_time_scale[match(State,hsmm$states$abbr)]  %>% str_c(.," states") %>% 
    factor(.,levels = unique(hsmm$states$group_sojourn_time_scale) %>% str_c(.," states")) 
) %>% arrange(group, State) %>% mutate(color = factor(color, levels = unique(color)))

g = ggplot(sojourn_long %>%  dplyr::filter(d > 1/2000, !(group == "- states")),aes(x = day, y = d, fill = color, col = color))
g = g + geom_area(position = "identity", alpha = 0.5)+
  ...
  expand_limits(x = 0)+
  ylab("pdf")+xlab("days")+
  scale_x_continuous(breaks = function(x){x = round(x); if(x[2]< 63){by = 7};if(x[2] %in% 63:200){by = 30};if(x[2]> 200){by = 90};return(seq(0,x[2],by = by))})+
  facet_wrap(group~., scale = "free", nrow = 4, dir = "h", strip.position = "top") #+ theme(legend.position = "bottom", legend.key.size = unit(0.3,"line"), legend.direction = "vertical",legend.box.margin = margin(t = -20))
g

ggsave(g, filename = str_c(IO$panels,"sojourn_distributions.pdf"),width = viz$full_width/4, height = viz$full_width/1.8, scale = 0.85)

g = ggplot(sojourn_long %>%  dplyr::filter(d > 1/10000), aes(x = day, y = d, fill = color, col = color))
g + geom_area()+
