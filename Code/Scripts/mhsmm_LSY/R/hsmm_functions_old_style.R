...
hsmmfit <- function(x,model,mstep=NULL,M=NA,maxit=100,lock.transition=FALSE,lock.d=FALSE, label = NULL,graphical=FALSE, debug = FALSE) {
  ...
  for(it in 1:maxit) {
    ...
    if(censoring){
      p = sapply(1:J,function(state) f(x,c = cens,state,new.model, w = new.model$weights[state,])) 
    }else{
    ...
    if(graphical) matplot(p, type = "l", col = hsmm$states$colors, lty = 1)
    ...
    new.parms.emission = mstep(x,state_wt_mod) # mstep function estimate the emission parameters by computing the values of the parameters of the distribution of the observations in which each observatuon is weighted by the probability of being in each state 
    for(state in unvisited_states){for(par.n in length(new.parms.emission)){ new.parms.emission[[par.n]][[state]] = old.parms.emission[[par.n]][[state]]}} # if some states are not visited, we dont want to change the emission probabilities of these states
    if(censoring){ # if some observations were always missing for some specific states, the new mus and sigmas are NAs. We thus want to replace the NAs by the old value
      for(state in setdiff(1:J, unvisited_states)){ if(debug){cat("\t\t state:",state,"\n")}
        for(par.n in 1:length(new.parms.emission)){ 
          if(names(new.parms.emission)[par.n] == "mu"){
            j = which(is.na(new.parms.emission[[par.n]][[state]])); if(debug){cat("\t\t\t sum(is.na(mu)):",length(j),"\n")}
            if(length(j)>0){new.parms.emission[[par.n]][[state]][j] = old.parms.emission[[par.n]][[state]][j]}
          }
          if(names(new.parms.emission)[par.n] == "sigma"){
            # NOT ANYMORE !!! we make sure to keep a diagonal sigma (conditionally independent variables)
            new.parms.emission[[par.n]][[state]] = diag(diag(new.parms.emission[[par.n]][[state]])) 
            
            # we remove the NAs
            j = is.na(new.parms.emission[[par.n]][[state]]) ; if(debug){cat("\t\t\t sum(is.na(sigma)):",sum(j),"\n")}
            if(sum(j)>0){new.parms.emission[[par.n]][[state]][j] = old.parms.emission[[par.n]][[state]][j]}
            # we remove the too small sigmas as they create issues in the E-step
            j = which(diag(new.parms.emission[[par.n]][[state]]) < 0.2) ; if(debug){cat("\t\t\t # of sigmas smaller than 0.2:",length(j),"\n")}
            if(length(j)>0){new.parms.emission[[par.n]][[state]][cbind(j,j)] = 0.2}
            # we remove the too big sigmas
            j = which(diag(new.parms.emission[[par.n]][[state]]) > 2) ; if(debug){cat("\t\t\t # of sigmas bigger than 2:",length(j),"\n")}
            if(length(j)>0){new.parms.emission[[par.n]][[state]][cbind(j,j)] = 2}
    ...
    if(debug){
      cat("old.model parms.emission sigmas:\n")
      for(si in 1:length(old.model$parms.emission$sigma)){
        cat("\t", si, " : \n\t\t", 
            old.model$parms.emission$sigma[[si]] %>%  diag() %>%  sprintf("%.4f",.),"\n\t\t",  
            new.model$parms.emission$sigma[[si]] %>%  diag() %>%  sprintf("%.4f",.),"\n")
      }
    }
  ...
  if(method=="viterbi") {
    ...
    for(i in 1:length(N)) {
      if(NCOL(x0)==1)    b = log(unlist(sapply(1:J,function(state) object$f(x0[(NN[i]+1):NN[i+1]],state,object$model))))
        ...
        ix = (NN[i]+1):NN[i+1]
        if(censoring) b = log(unlist(sapply(1:J,function(state) object$f(x0[ix,],cens[ix,],state,object$model,w = object$model$weights[state,])))) 
...
predict.hsmmspec <- function(object,newdata,method="viterbi",M=NA,debug = FALSE,...) {
  ...
  class(object2) <- "hsmm"
  predict(object2,newdata,method, debug = debug)    
}
