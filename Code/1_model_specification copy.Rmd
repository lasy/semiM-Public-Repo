...
# Hidden semi-Markov model for labelling fertility-app data with reproductive events
...
R_model$states = bind_rows(
  data.frame(
    names = c("Menses",
              ...
              "delayed Ovulation",
              "Ovu - 1","Ovulation","Ovu + 2",
              "Luteal",
              "Anovulatory cycle",
              "Anovulatory with bleeding"), 
    abbr = c("M","lE","hE","dO","preO","O","postO","Lut","Ano","AB"), 
    colors = c("red3","skyblue1","skyblue2","skyblue3","blue","black","gold3","gold1","gray","salmon"), 
    group = c(rep("Ovulatory",8), "Anovulatory","Anovulatory"),
    ...
    description = c(
      ...
      "One-day state if ovulation is abnormaly delayed (typically observed in PCOS patients).",
...
R_model$sojourn["PL", (5*31):length(x)] = 0
# L
...
# BF
R_model$sojourn["BF",] =  dgamma(x, shape = 2.5, scale = 300) 
...
em_parms$LH$params$probs[,"dO"] = c(0.5,0.5)
...
# fertile mucus states
mucus_probs[, c("dO")] = fertile
...
temp_sd[c("M","L","PB2","PB3","B","PP","BF")] = 0.5
...
p[c("hE","dO","preO","O","postO","L")] = 0.3 # most people track ~2/3 days in these states 
...
q["LH",] = 0.99
q["LH",c("hE","dO","preO","O","postO")] = 0.5

q["mucus",] = 0.5
q["mucus",c("hE","preO","O","postO")] = 0.4
q["mucus",c("dO")] = 0.1 # that's how we know this is happening
...
### Specifying a `hsmm` with weak sojourn priors
...
```{r R-model-specify-weak-HSMM}

R_bad_hsmm = specify_hsmm(
  J = R_model$n_states,
  state_names = R_model$states$abbr,
  state_colors = R_model$state$colors,
  init = R_model$init,
...
```{r R-model-simulate-weak-HSMM}

X_bad_hsmm = simulate_hsmm(R_bad_hsmm, seed = 22, n_state_transitions = 150)
plot_hsmm_seq(X_bad_hsmm, model = R_bad_hsmm)
...
```{r R-weak-HSMM-model-save}

save(R_bad_hsmm, file = paste0("../Data/models/R_bad_hsmm.Rdata"))
